...
- **`context/`** — API specs (OpenAPI), reference docs, and anything that helps you (or your AI) understand the API
  under test. The `glubean context` command reads from here to generate AI-optimized context.
...
> Read `context/openapi.sample.json` and `AGENTS.md`. Generate Glubean tests in `tests/smoke.test.ts` that cover all GET
> endpoints with 200 responses. Use the builder API with `.step()` for multi-step flows. Use `ctx.http` for requests
> (auto-traces every call). Add assertions for status code and basic response schema. Group tests by tag.
